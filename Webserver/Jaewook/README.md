...
| 현재 Spring Boot 서버에서 실제 실행되는 화면 | [`../haktae/demo/src/main/resources/static/`](../haktae/demo/src/main/resources/static/) |
...
디자인 원본은 `user-ui-template`에 보관하고, Spring Boot 서버에서 실제 실행되는 복사본은 `../haktae/demo/src/main/resources/static`에 둡니다. 두 위치의 네 파일은 2026-08-27 기준으로 동일합니다.

UI를 수정할 때는 디자인 원본과 서버 적용본이 서로 달라지지 않도록 같은 변경을 반영하고 함께 확인합니다.

## 현재 서버 연동 규격

브라우저는 웹을 제공한 Spring Boot 서버의 상대 주소를 사용합니다. IP 주소와 로봇 좌표는 UI 파일에 저장하지 않습니다.

| 기능 | 요청 또는 상태 |
| --- | --- |
| 목적지 전송 | `POST /api/navigation` |
| 안내 취소 | `POST /api/command` |
| 수동모드·주행 상태 확인 | `GET /api/status` |

목적지 ID는 `toilet`, `301`, `302`, `elevator`를 사용합니다. 서버가 목적지 ID에 맞는 실제 좌표를 관리합니다.

## 작업할 때 주의할 점

- 실제 배포 서버 코드는 `haktae/demo`에 있습니다.
